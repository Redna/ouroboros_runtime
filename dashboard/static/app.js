...
function createLogEntryElement(entry) {
    ...
    if (hudMatch) {
        updateHUD(parseInt(hudMatch[1]), parseInt(hudMatch[2]), parseInt(hudMatch[3]));
    }
    ...
    if (entry.tool_calls && entry.tool_calls.length > 0) {
        entry.tool_calls.forEach(tc => {
            ...
            toolDiv.innerHTML = `<span class="tool-name">🛠️ ${tc.function.name}</span>`;
            if (args && args !== '{}') {
                const details = document.createElement('details');
                details.innerHTML = `<summary>View Parameters</summary><pre><code>${escapeHtml(args)}</code></pre>`;
    ...
    if (content) {
        ...
        // Expandable logic for long content (> 500 chars)
        if (content.length > 500) {
            const details = document.createElement('details');
            const summary = document.createElement('summary');
            summary.innerText = "... (click to expand long message)";
        ...
        contentContainer.innerHTML = '<span style="color: #444; font-style: italic;">[Empty message]</span>';
    }
